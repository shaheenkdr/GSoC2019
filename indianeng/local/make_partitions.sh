...
if [ $stage -eq 8 ]; then
  utils/combine_data.sh $data_dir/fisher_swbd_tedlium_wsj_hub4_libri460 \
    $data_dir/fisher_swbd_tedlium data/{librispeech_100,librispeech_360}/train \
    || { echo "Failed to combine data"; exit 1; }
  utils/data/remove_dup_utts.sh 300 $data_dir/fisher_swbd_tedlium_wsj_hub4_libri460 $data_dir/tri4_ali
  ln -nfs tri4_ali $data_dir/tri5b
fi
