...
dir=${dir}$suffix
train_set=multi_a/${gmm}
build_tree_ali_dir=exp/multi_a/${gmm}_ali
...
if [ $stage -le 9 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" data/$train_set \
    data/lang_nosp_fsh_sw1_tg exp/multi_a/$gmm exp/multi_a/${gmm}_lats_nodup$suffix
...
if [ $stage -le 10 ]; then
  ...
  rm -rf $lang
  cp -r data/lang_nosp_fsh_sw1_tg $lang
...
if [ $stage -le 11 ]; then
  ...
  steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      ...
      --cmd "$train_cmd" 11000 data/$train_set $lang exp/multi_a/tri6a $treedir
fi

echo "GPU BASED TRAINING NOW!!!"
#exit 0
...
if [ $stage -le 13 ]; then
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --tree-dir $treedir \
    --lat-dir exp/multi_a/tri6a_lats_nodup \
    --dir $dir  || exit 1;
fi
